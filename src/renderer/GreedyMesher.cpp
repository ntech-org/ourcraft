...
struct FaceMaskCell { bool visible = false; int textureIndex = 0; float depth = 0.0f; };
...
FaceMaskCell makeMaskCell(std::uint8_t id, int face, float depth) {
    if (!isGreedyRenderable(id)) return {};
    return { true, Block::blocksList[id]->getTexture(face), depth };
}

bool sameCell(const FaceMaskCell& l, const FaceMaskCell& r) {
    return l.visible == r.visible && l.textureIndex == r.textureIndex && std::abs(l.depth - r.depth) < 0.01f;
}

void appendVertex(ChunkMeshData::Pass& p, float x, float y, float z, float u, float v, int tex, FaceDirection dir, float depth) {
    p.vertices.push_back({x, y, z, u, v, kWhiteColor, (std::uint32_t)tex, (std::uint32_t)dir, -1000.0f, 0.0f, depth});
}
...
void ChunkMesher::greedyMeshTopBottom(ChunkMeshData& md, const IBlockAccess& n, int si, int cx, int cz, bool up) {
    ...
    for (int ly = 0; ly < 16; ++ly) {
        ...
        for (int x = 0; x < 16; ++x) for (int z = 0; z < 16; ++z) {
            ...
            if (!shouldCull(bid, nid)) mask[x + z * 16] = makeMaskCell(bid, f, getWaterDepth(n, x, gy + (up ? 1 : 0), z));
        }
        for (int z = 0; z < 16; ++z) for (int x = 0; x < 16;) {
            ...
            if (up) {
                appendVertex(md.opaque, x1, yq, z1, (float)w, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x1, yq, z0, (float)w, 0, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, x0, yq, z0, 0, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x0, yq, z1, 0, (float)h, c.textureIndex, dir, c.depth);
            } else {
                appendVertex(md.opaque, x0, yq, z1, 0, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x0, yq, z0, 0, 0, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, x1, yq, z0, (float)w, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x1, yq, z1, (float)w, (float)h, c.textureIndex, dir, c.depth);
            }
...
void ChunkMesher::greedyMeshNorthSouth(ChunkMeshData& md, const IBlockAccess& n, int si, int cx, int cz, bool south) {
    ...
    for (int lz = 0; lz < 16; ++lz) {
        ...
        for (int x = 0; x < 16; ++x) for (int y = 0; y < 16; ++y) {
            ...
            if (!shouldCull(bid, nid)) mask[x + y * 16] = makeMaskCell(bid, f, getWaterDepth(n, x, gy, lz + (south ? 1 : 0)));
        }
        for (int y = 0; y < 16; ++y) for (int x = 0; x < 16;) {
            ...
            if (!south) {
                appendVertex(md.opaque, x0, y1, zq, 0, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x1, y1, zq, (float)w, 0, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, x1, y0, zq, (float)w, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x0, y0, zq, 0, (float)h, c.textureIndex, dir, c.depth);
            } else {
                appendVertex(md.opaque, x0, y1, zq, 0, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x0, y0, zq, 0, (float)h, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, x1, y0, zq, (float)w, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, x1, y1, zq, (float)w, 0, c.textureIndex, dir, c.depth);
            }
            appendIndices(md.opaque); x += w;
        }
    }
...
void ChunkMesher::greedyMeshWestEast(ChunkMeshData& md, const IBlockAccess& n, int si, int cx, int cz, bool east) {
    ...
    for (int lx = 0; lx < 16; ++lx) {
        ...
        for (int z = 0; z < 16; ++z) for (int y = 0; y < 16; ++y) {
            ...
            if (!shouldCull(bid, nid)) mask[z + y * 16] = makeMaskCell(bid, f, getWaterDepth(n, lx + (east ? 1 : 0), gy, z));
        }
        for (int y = 0; y < 16; ++y) for (int z = 0; z < 16;) {
            ...
            if (!east) {
                appendVertex(md.opaque, xq, y1, z1, (float)w, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, xq, y1, z0, 0, 0, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, xq, y0, z0, 0, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, xq, y0, z1, (float)w, (float)h, c.textureIndex, dir, c.depth);
            } else {
                appendVertex(md.opaque, xq, y0, z1, 0, (float)h, c.textureIndex, dir, c.depth); appendVertex(md.opaque, xq, y0, z0, (float)w, (float)h, c.textureIndex, dir, c.depth);
                appendVertex(md.opaque, xq, y1, z0, (float)w, 0, c.textureIndex, dir, c.depth); appendVertex(md.opaque, xq, y1, z1, 0, 0, c.textureIndex, dir, c.depth);
            }
